...
`WARN` is currently defined as a general warning state in the shared status model.

A firmware node can report WARN through its status register, but the specific device-level conditions that should trigger WARN are determined by each node implementation and are not yet standardized across all module types.

The display-state model can also incorporate warnings generated by the monitoring application. The current mock reader uses this mechanism to test how an external warning can elevate a READY or BUSY node to WARN.

Real power-monitor measurements are displayed and logged separately. Automatic mapping of INA219 measurements to individual node WARN states, including threshold and port-to-node rules, has not yet been implemented.
